    ...
    private func filterPill(id: String?, label: String, count: Int) -> some View {
        ...
        Button {
            ...
            HStack(spacing: 7) {
                ...
                if count > 0 {
                    Text("\(count)")
                        .font(Theme.Type.rounded(size: 11, weight: .semibold))
                        .monospacedDigit()
                        .foregroundStyle(
            ...
            VStack(alignment: .leading, spacing: 3) {
                HStack(alignment: .firstTextBaseline) {
                    Text(conv.title?.nilIfEmpty ?? "新对话")
                        .font(Theme.Type.serif(size: 16, weight: .semibold))
                        .foregroundStyle(
        ...
        if model.ws.status != .connected {
            HStack(spacing: 8) {
                ...
                Text(model.ws.status == .connecting ? "正在连接" : "未连接")
                    .font(Theme.Type.rounded(size: 12, weight: .medium))
                    .foregroundStyle(Theme.Palette.inkMuted)
            }
            ...
            .animation(.easeInOut(duration: 0.22), value: model.ws.status)
        }
    }
}
